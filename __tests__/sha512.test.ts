...
import { SHA512, SHA512Algo, WordArray, HmacSHA512, Hex, HMAC } from '../src/index';

describe('sha512', () => {
  it('vector 1', () => {
    expect(SHA512('').toString())
  ...
  it('vector 2', () => {
    expect(SHA512('The quick brown fox jumps over the lazy dog').toString())
  ...
  it('update and long message', () => {
    const sha512 = SHA512Algo.create();
  ...
  it('clone', () => {
    ...
    expect((sha512.update('a').clone() as SHA512Algo).finalize().toString()).toBe(SHA512('a').toString());
    expect((sha512.update('b').clone() as SHA512Algo).finalize().toString()).toBe(SHA512('ab').toString());
    expect((sha512.update('c').clone() as SHA512Algo).finalize().toString()).toBe(SHA512('abc').toString());
  });
  ...
  it('helper', () => {
    expect(SHA512('').toString()).toBe(SHA512Algo.create().finalize('').toString());
  });

  it('hmacHelper', () => {
    expect(HmacSHA512('Hi There', Hex.parse('0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b')).toString())
      .toBe(HMAC.create(SHA512Algo, Hex.parse('0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b')).finalize('Hi There').toString());
  });
});
