...
describe('rabbit-legacy', () => {
  ...
  it('vector 4', () => {
    expect(C.RabbitLegacy.encrypt(C.enc.Hex.parse('00000000000000000000000000000000'), C.enc.Hex.parse('00000000000000000000000000000000'), { iv: C.enc.Hex.parse('0000000000000000') }).ciphertext.toString())
      .toBe('edb70567375dcd7cd89554f85e27a7c6');
  });

  it('vector 5', () => {
    expect(C.RabbitLegacy.encrypt(C.enc.Hex.parse('00000000000000000000000000000000'), C.enc.Hex.parse('00000000000000000000000000000000'), { iv: C.enc.Hex.parse('597e26c175f573c3') }).ciphertext.toString())
      .toBe('6d7d012292ccdce0e2120058b94ecd1f');
  });

  it('vector 6', () => {
    expect(C.RabbitLegacy.encrypt(C.enc.Hex.parse('00000000000000000000000000000000'), C.enc.Hex.parse('00000000000000000000000000000000'), { iv: C.enc.Hex.parse('2717f4d21a56eba6') }).ciphertext.toString())
  ...
  it('input integrity', () => {
    ...
    C.RabbitLegacy.encrypt(message, key, { iv });
  ...
  it('test helper', () => {
    ...
    expect(C.RabbitLegacy.encrypt('Hi There', C.SHA256('Jefe'), { mode: C.mode.ECB, padding: C.pad.NoPadding }).ciphertext.toString())
      .toBe(C.algo.RabbitLegacy.createEncryptor(C.SHA256('Jefe'), { mode: C.mode.ECB, padding: C.pad.NoPadding }).finalize('Hi There').toString());
    expect(C.RabbitLegacy.encrypt('Hi There', C.SHA256('Jefe'), { mode: C.mode.ECB, padding: C.pad.NoPadding }).toString())
      .toBe(C.lib.SerializableCipher.encrypt(C.algo.RabbitLegacy, 'Hi There', C.SHA256('Jefe'), { mode: C.mode.ECB, padding: C.pad.NoPadding }).toString());
    expect(C.RabbitLegacy.encrypt('Hi There', 'Jefe', { mode: C.mode.ECB, padding: C.pad.NoPadding }).toString())
      .toBe(C.lib.PasswordBasedCipher.encrypt(C.algo.RabbitLegacy, 'Hi There', 'Jefe', { mode: C.mode.ECB, padding: C.pad.NoPadding }).toString());
