...
import { SHA384, SHA384Algo, WordArray, HmacSHA384, Hex, HMAC } from '../src/index';

describe('sha384', () => {
  it('vector 1', () => {
    expect(SHA384('').toString())
      .toBe('38b060a751ac96384cd9327eb1b1e36a21fdb71114be07434c0cc7bf63f6e1da274edebfe76f65fbd51ad2f14898b95b');
  });

  it('vector 2', () => {
    expect(SHA384('The quick brown fox jumps over the lazy dog').toString())
  ...
  it('update and long message', () => {
    const sha384 = SHA384Algo.create();
  ...
  it('clone', () => {
    ...
    expect((sha384.update('a').clone() as SHA384Algo).finalize().toString()).toBe(SHA384('a').toString());
    expect((sha384.update('b').clone() as SHA384Algo).finalize().toString()).toBe(SHA384('ab').toString());
    expect((sha384.update('c').clone() as SHA384Algo).finalize().toString()).toBe(SHA384('abc').toString());
  });
  ...
  it('helper', () => {
    expect(SHA384('').toString()).toBe(SHA384Algo.create().finalize('').toString());
  });

  it('hmacHelper', () => {
    expect(HmacSHA384('Hi There', Hex.parse('0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b')).toString())
      .toBe(HMAC.create(SHA384Algo, Hex.parse('0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b')).finalize('Hi There').toString());
  });
});
