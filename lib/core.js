...
export class WordArray extends Base {
   ...
   */
  toArrayBuffer() {
    const arr = new Uint8Array(this.sigBytes);
    return arr.map((_, i) => (this.words[i >>> 2] >>> (24 - (i % 4) * 8)) & 0xff).buffer;
  }
...
export class BufferedBlockAlgorithm extends Base {
  ...
  _append(data) {
    ...
    } else if (isArrayBuffer(m_data)) {
      m_data = WordArray.create(m_data);
    }
...
export class HMAC extends Base {
  ...
  constructor(SubHasher, key) {
    ...
    } else if (isArrayBuffer(_key)) {
      _key = WordArray.create(_key);
    }
